...
class LoginManager:
    ...
    def _get_login_page(self):
        ...
        # The login page may only be a stub that redirects to the real portal page
        # through a <meta http-equiv="refresh"> tag, which requests does not follow.
        redirect = re.search(
            'http-equiv="refresh"[^>]*url=([^"\'>\\s]+)', self._page_response.text, re.IGNORECASE
        )
        if redirect:
            url = redirect.group(1).replace('&amp;', '&')
            if not url.startswith('http'):
                url = self.args['url'] + url
            self._page_response = requests.get(url, headers=header)
    ...
    def _resolve_ip_from_login_page(self):
        # Old portal pages embed the ip in a hidden input, new ones in a JS `var CONFIG = {...}`
        for pattern in (
                'id="user_ip" value="([\\d.]+)"',
                '\\bip\\s*:\\s*["\']([\\d.]+)["\']',
        ):
            match = re.search(pattern, self._page_response.text)
            if match:
                self.ip = match.group(1)
                return
        raise ValueError("Cannot find local ip in login page html, maybe the srun portal is updated")
